...
```
feat(reports): add daily sales by operating point
fix(api-client): map 409 problem responses
docs: document reporting hierarchy
...
1. **No business rules in PHP.** Validation that matters, pricing, stock, payments,
   refunds, tickets, bookings, memberships and order state live in the 007 Resort & Spa API. PHP
...
6. **Authorization** comes from API-issued permissions; never hard-code role checks as the
   source of truth.
7. **No secrets.** Never commit `.env`, keys, tokens, certificates or real customer data.
