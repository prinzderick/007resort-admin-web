<x-layouts.app title="Payment timing">
    ...
    <x-card flush>
        ...
        @forelse ($rules as $r)
            <tr><td class="font-medium"><a class="underline" href="{{ route('setup.facilities', ['facility' => $r['facility']['id'] ?? '']) }}">{{ $r['facility']['name'] ?? '' }}</a></td><td class="text-right"><x-money :value="$r['rules']['approvalThresholdAmount'] ?? '0'" /></td>
