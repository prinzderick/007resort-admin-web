            ...
            <div class="table-scroll"><table class="data-table" data-testid="facility-list">
                ...
                @forelse ($rows as $r)
                    <tr data-row>
                        ...
                        <td class="max-w-md text-xs text-stone-500">{{ implode(', ', array_map(fn ($c) => $capLabels[$c] ?? ucwords(strtolower(str_replace('_', ' ', $c))), array_slice((array) ($r['capabilities'] ?? []), 0, 6))) }}@if (count((array) ($r['capabilities'] ?? [])) > 6) +{{ count($r['capabilities']) - 6 }} more @endif</td>
