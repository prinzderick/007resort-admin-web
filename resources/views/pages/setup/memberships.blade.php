...
<x-layouts.app title="Membership plans">
    ...
    @if ($canEdit)
        ...
        <x-card :title="$cur ? 'Edit plan' : 'New plan'" id="plan-form">
            <form method="POST" action="{{ $cur ? route('setup.memberships.update', $cur['id']) : route('setup.memberships.store') }}" class="grid gap-x-4 sm:grid-cols-2">
                ...
                <div class="sm:col-span-2"><x-btn>{{ $cur ? 'Save plan' : 'Create plan' }}</x-btn> @if ($cur)<x-btn variant="secondary" :href="route('setup.memberships')">Cancel</x-btn>@endif</div>
            </form>
        </x-card>
    @endif
</x-layouts.app>
