<x-layouts.app title="KDS routing">
    ...
    <x-card title="Stations" flush x-data="tableTools">
        <x-table-tools />
        <x-fetch :of="$stations" what="Stations" />
        <x-fetch :of="$products" what="Product routing" />
        @if ($stations->ok())<div class="table-scroll"><table class="data-table" data-testid="stations"><thead><tr><th>Station</th><th>Kind</th><th>Active</th><th>Products routed</th></tr></thead><tbody>
        @forelse ($stations->items() as $s)<tr data-row><td class="font-medium">{{ $s['name'] ?? '' }}</td><td>{{ $s['kind'] ?? '' }}</td><td><x-badge :tone="($s['active'] ?? true) ? 'good' : 'default'">{{ ($s['active'] ?? true) ? 'Active' : 'Off' }}</x-badge></td><td class="text-sm">{{ implode(', ', $byStation[$s['name'] ?? ''] ?? []) ?: '-' }}</td></tr>@empty<tr><td colspan="4" class="text-center text-stone-500">No stations.</td></tr>@endforelse
