    ...
    @if ($canUpload)
        ...
        <x-dialog name="delete-media" title="Delete this picture?" subtitle="Pictures that are still used on the website cannot be deleted." maxWidth="max-w-lg">
            <div x-data="{ usage: null, err: '', load() { this.usage = null; this.err = ''; fetch('{{ url('/website/media') }}/' + payload.id + '/usage', { headers: { Accept: 'application/json' } }).then(r => r.json()).then(j => { this.usage = j; }).catch(() => { this.err = 'Could not check where this picture is used.'; }); } }" x-init="load()">
                ...
                <template x-if="usage && usage.usageCount > 0"><div data-testid="usage-blockers"><p class="mb-2 text-sm font-medium text-amber-800">It is used in <span x-text="usage.usageCount"></span> place(s). Replace it there first:</p><ul class="list-disc space-y-0.5 pl-5 text-sm"><template x-for="u in usage.usage" :key="u.type + u.id + u.field"><li><span x-text="u.type.replace(/([A-Z])/g, ' $1').replace('_', ' ').toLowerCase()"></span>: <span x-text="u.label"></span></li></template></ul><div class="mt-4 flex justify-end"><x-btn type="button" variant="secondary" @click="open = false">Close</x-btn></div></div></template>
