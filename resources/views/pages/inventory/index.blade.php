<x-layouts.app title="Inventory">
    ...
    <x-card title="Balances" flush>
        <x-fetch :of="$balances" what="Balances" />
        @if ($balances->ok())
            <div class="overflow-x-auto"><table class="data-table" data-testid="balances">
                ...
                <tbody>
                @forelse ($rows as $r)
                    <tr class="{{ $r['low'] ? 'bg-red-50' : '' }}"><td>{{ $r['itemName'] ?? $r['itemId'] }}</td><td>{{ $r['location'] }}</td><td class="text-right tabular-nums {{ $r['low'] ? 'font-semibold text-red-800' : '' }}">{{ $r['quantity'] }} {{ $r['unit'] ?? '' }}</td><td class="text-right tabular-nums">{{ $r['reorderLevel'] ?? '-' }}</td><td><x-time :at="$r['updatedAt'] ?? null" ago /></td></tr>
                @empty<tr><td colspan="5" class="text-center text-stone-500">No balances.</td></tr>@endforelse
                </tbody></table></div>
        @endif
    </x-card>
    <x-pending-api :items="['Movement history list (receipts, transfers, sales consumption, wastage, adjustments) - no GET /inventory/movements in the contract yet', 'Saved count sheets and a variance report across counts - only the result of creating/posting a count is returned today']" />
</x-layouts.app>
