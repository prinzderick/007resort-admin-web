<x-layouts.app title="Payments">
    ...
    <x-card flush>
        ...
        @if ($payments->ok())
            <div class="overflow-x-auto"><table class="data-table" data-testid="payments-table">
                ...
                @forelse ($payments->items() as $p)
                    <tr><td><x-time :at="$p['createdAt']" /></td><td>{{ str_replace('_', ' ', $p['tenderType']) }}@if (($p['provider'] ?? 'MANUAL') !== 'MANUAL')<div class="text-xs text-stone-500">{{ $p['provider'] }}</div>@endif</td>
                        <td class="text-xs">{{ $p['providerReference'] ?? $p['reference'] ?? '' }}</td><td><x-badge :status="$p['status']" /></td>
                        <td class="text-right"><x-money :value="$p['amount']" /></td><td class="text-right"><x-money :value="$p['refundedAmount'] ?? '0'" /></td>
            ...
            @if ($payments->next())<div class="p-3"><x-btn variant="secondary" :href="request()->fullUrlWithQuery(['cursor' => $payments->next()])">Next page</x-btn></div>@endif
        @endif
    </x-card>
</x-layouts.app>
