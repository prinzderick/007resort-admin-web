        ...
        @if ($payments->ok())
            <div class="overflow-x-auto"><table class="data-table" data-testid="payments-table">
                ...
                @forelse ($payments->items() as $p)
                    <tr><td><x-time :at="$p['createdAt'] ?? null" /></td><td>{{ str_replace('_', ' ', $p['tenderType'] ?? '') }}@if (($p['provider'] ?? 'MANUAL') !== 'MANUAL')<div class="text-xs text-stone-500">{{ $p['provider'] }}</div>@endif</td>
