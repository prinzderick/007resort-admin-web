<x-layouts.app title="Reconciliation">
    ...
    @if ($payments->ok())
        <x-card title="Captured by method" flush x-data="tableTools">
        <x-table-tools /><div class="table-scroll"><table class="data-table" data-testid="recon-methods"><thead><tr><th>Method</th><th class="text-right">Payments</th><th class="text-right">Captured</th><th class="text-right">Refunded</th><th class="text-right">Net</th></tr></thead><tbody>
            @forelse ($byMethod as $m => $v)<tr data-row><td>{{ str_replace('_', ' ', $m) }}</td><td class="text-right">{{ $v['count'] }}</td><td class="text-right"><x-money :value="$v['captured']" /></td><td class="text-right"><x-money :value="$v['refunded']" /></td><td class="text-right"><x-money :value="\App\Support\Money::sub($v['captured'], $v['refunded'])" /></td></tr>@empty<tr><td colspan="5" class="text-center text-stone-500">No payments in range.</td></tr>@endforelse
        </tbody></table></div></x-card>

        <x-card title="Provider transactions to match" flush x-data="tableTools">
        <x-table-tools />
            ...
            <div class="table-scroll"><table class="data-table"><thead><tr><th>Reference</th><th>Provider</th><th>Status</th><th class="text-right">Amount</th><th></th></tr></thead><tbody>
            @forelse ($provider as $p)
                <tr data-row><td class="text-xs">{{ $p['providerReference'] ?? '' }}</td><td>{{ $p['provider'] ?? '' }}</td><td><x-badge :status="$p['status'] ?? 'UNKNOWN'" /></td><td class="text-right"><x-money :value="$p['amount'] ?? '0'" /></td>
