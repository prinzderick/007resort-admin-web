...
<x-layouts.app title="Membership plans">
    ...
    <x-card title="Plans" flush>
        ...
        @if ($plans->ok())
            ...
            @forelse ($plans->items() as $p)
                <tr><td class="font-medium">{{ $p['name'] }}</td><td>{{ $p['durationDays'] }} days</td><td class="text-right"><x-money :value="$p['price']" /></td><td>{{ $p['visitLimit'] ?? 'Unlimited' }}</td>
                    <td>{{ empty($p['facilityIds']) ? 'Whole property' : count($p['facilityIds']).' facility(ies)' }}</td><td><x-badge :tone="($p['active'] ?? true) ? 'good' : 'default'">{{ ($p['active'] ?? true) ? 'Active' : 'Inactive' }}</x-badge></td>
                    <td>@if ($canEdit)<a class="underline" href="{{ route('config.memberships', ['edit' => $p['id']]) }}">Edit</a>@endif</td></tr>
    ...
    @if ($canEdit)
        ...
        <x-card :title="$cur ? 'Edit plan' : 'New plan'">
