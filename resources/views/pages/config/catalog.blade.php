<x-layouts.app title="Catalog">
        ...
        @if ($products->ok())
            ...
            @forelse ($products->items() as $p)
                ...
                <tr class="{{ ($p['active'] ?? true) ? '' : 'opacity-60' }}"><td class="font-medium">{{ $p['name'] ?? '' }}@if (! ($p['active'] ?? true)) <x-badge>inactive</x-badge>@endif<div class="text-xs font-normal text-stone-500">{{ $p['sku'] ?? '' }}</div></td><td>{{ $catNames[$p['categoryId'] ?? ''] ?? '' }}</td><td>{{ $p['kind'] ?? '' }}</td>
                    <td class="text-right">
                        @if ($canPrice && $pid !== '')
                            <form method="POST" action="{{ route('config.catalog.price', $pid) }}" class="flex items-center justify-end gap-1">@csrf @method('PUT')<input type="hidden" name="facilityId" value="{{ $facilityId }}">
                    ...
                    <td>@if (auth_staff()->can('catalog.availability.manage') && $facilityId && $pid !== '')
                        <form method="POST" action="{{ route('config.availability', $pid) }}" class="flex items-center gap-2">@csrf @method('PUT')<input type="hidden" name="facilityId" value="{{ $facilityId }}"><input type="hidden" name="available" value="{{ $on ? 0 : 1 }}">
                    ...
                    @if ($canManage)<td>@if ($pid !== '')<a class="underline" href="{{ route('config.catalog', ['facility' => $facilityId, 'edit' => $pid]) }}#product-form">Edit</a>@endif</td>@endif</tr>
    ...
    @if ($canManage)
        ...
        <x-card :title="$edit ? 'Edit product' : 'New product'" id="product-form">
            <form method="POST" action="{{ $edit ? route('config.catalog.product.update', $edit['id']) : route('config.catalog.product.create') }}" class="grid gap-x-4 sm:grid-cols-2" data-testid="product-form">
                ...
                <div class="sm:col-span-2"><x-btn>{{ $edit ? 'Save product' : 'Create product' }}</x-btn> @if ($edit)<x-btn variant="secondary" :href="route('config.catalog', ['facility' => $facilityId])">Cancel</x-btn>@endif</div>
            </form>
        </x-card>
    @endif
    ...
    <x-card title="Categories" flush>
        ...
        @if ($canManage)
            <form method="POST" action="{{ route('config.catalog.category.create') }}" class="flex flex-wrap items-end gap-3 border-t border-stone-100 p-4">@csrf<input type="hidden" name="facilityId" value="{{ $facilityId }}">
