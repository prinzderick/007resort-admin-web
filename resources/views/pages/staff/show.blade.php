@php $displayName = $member['displayName'] ?? trim(($member['firstName'] ?? '').' '.($member['lastName'] ?? '')) ?: 'Staff member'; @endphp
<x-layouts.app :title="$displayName">
    <x-page-header :title="$displayName" :subtitle="'Staff no. '.($member['staffNumber'] ?? '-')">
    ...
    <div class="grid gap-5 lg:grid-cols-2">
        <x-card title="Profile">
            <form method="POST" action="{{ route('staff.update', $member['id'] ?? '') }}">
                ...
                <x-field name="status" label="Status" :options="['ACTIVE' => 'Active', 'SUSPENDED' => 'Suspended', 'TERMINATED' => 'Terminated']" :value="$member['status'] ?? 'ACTIVE'" required />
    ...
    <x-card title="Roles and scopes" flush>
        ...
        @if ($assign->ok())
            ...
            @forelse ($assign->items() as $a)
                ...
                <tr><td>{{ $roleNames[$a['roleId'] ?? ''] ?? $a['roleCode'] ?? $a['roleId'] ?? '' }}</td><td>{{ $a['scopeType'] ?? '' }}@if (($a['scopeType'] ?? '') === 'FACILITY' && ! empty($a['scopeId']))<div class="text-xs text-stone-500">{{ collect($facilities)->firstWhere('id', $a['scopeId'])['name'] ?? '' }}</div>@endif</td><td><x-time :at="$a['grantedAt'] ?? null" /></td>
                    <td>@if (! empty($a['id']) && auth_staff()->can('role_assignment.manage'))<form method="POST" action="{{ route('staff.role.revoke', [$member['id'], $a['id']]) }}">@csrf @method('DELETE')<button class="text-sm text-red-800 underline" onclick="return confirm('Revoke this role?')">Revoke</button></form>@endif</td></tr>
                @endif
        ...
        @if (auth_staff()->can('role_assignment.manage') && $roles->ok())
            ...
            <form method="POST" action="{{ route('staff.role.grant', $member['id']) }}" class="grid gap-x-3 border-t border-stone-100 p-4 sm:grid-cols-4">
                ...
                <x-field name="scopeId" label="Scope target" :options="$scopeOptions" required hint="Facility for FACILITY scope, the site entry for SITE, the organization entry for ORGANIZATION." />
