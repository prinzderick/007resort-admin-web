<x-layouts.app title="Reports">
    ...
    <x-card title="Facilities on {{ $date }}" flush x-data="tableTools">
        ...
        <div class="table-scroll"><table class="data-table" data-testid="property-table">
            ...
            <tbody>
            @forelse ($rows as $r)
                <tr data-row><td><a class="font-medium underline" href="{{ route('reports.facility', ['facility' => $r['facility']['id'], 'date' => $date]) }}">{{ $r['facility']['name'] ?? '?' }}</a></td>
        ...
        @if ($period->ok())
            ...
            <div class="grid gap-5 lg:grid-cols-2">
                <div><div class="mb-1 text-xs font-semibold uppercase text-stone-500">By facility</div>
                    <div class="table-scroll"><table class="data-table"><thead><tr><th>Facility</th><th class="text-right">Orders</th><th class="text-right">Revenue</th></tr></thead><tbody>
                ...
                <div><div class="mb-1 text-xs font-semibold uppercase text-stone-500">Payments by method (captured, refunded, net)</div>
                    <div class="table-scroll"><table class="data-table"><thead><tr><th>Method</th><th class="text-right">Count</th><th class="text-right">Captured</th><th class="text-right">Refunded</th><th class="text-right">Net</th></tr></thead><tbody>
            ...
            @if (! empty($data['byOperatingPoint']))
                <div class="mt-4"><div class="mb-1 text-xs font-semibold uppercase text-stone-500">By operating point</div>
                    <div class="table-scroll"><table class="data-table"><thead><tr><th>Operating point</th><th class="text-right">Orders</th><th class="text-right">Revenue</th></tr></thead><tbody>
