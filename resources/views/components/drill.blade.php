...
<nav class="mb-4 flex flex-wrap items-center gap-1 text-xs" aria-label="Drill-down">
    @foreach ($levels as $i => $l)
        <span class="rounded-full px-2.5 py-1 {{ $l === $at ? 'bg-brand-600 text-white' : (isset($pending[$l]) ? 'border border-dashed border-stone-300 text-stone-400' : 'border border-stone-300 text-stone-600') }}"
