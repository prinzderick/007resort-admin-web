@props(['label', 'value', 'hint' => null, 'tone' => 'default', 'delta' => null, 'deltaLabel' => 'vs previous period', 'invert' => false, 'spark' => null, 'prev' => null])
@php
...
<div class="rounded-xl border {{ $ring }} bg-white px-5 py-4 shadow-[var(--shadow-card)]" data-component="stat-card">
    ...
    @elseif ($hint)<div class="mt-1.5 text-xs text-stone-500">{{ $hint }}</div>
    @elseif ($prev !== null)<div class="mt-1.5 text-xs text-stone-500" data-testid="prev">Before: {{ $prev }} &middot; {{ $deltaLabel }}</div>@endif
</div>
