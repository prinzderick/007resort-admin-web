...
- [ ] Title follows Conventional Commits (e.g. `feat(reports): add daily sales view`)
- [ ] No business rules or business-data writes in PHP - all business operations call the Otueke API (`/api/v1/...`)
...
- [ ] Mutating API calls send an `Idempotency-Key` (via `OtuekeApiClient`)
